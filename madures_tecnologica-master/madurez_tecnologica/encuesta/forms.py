...
class NameForm(forms.Form):
    ...
    GAD.widget.attrs.update(
        {"class": "form-control form-control-user", "placeholder": "Ingresar municipio"}
    )
...
class Name2Form(forms.Form):
    ...
    def __init__(self, question):
        ...
        valor = GadsServicios.objects.prefetch_related("servicios").filter(
            gad_id=question
        )
...
class PreguntaTop(forms.Form):
    ...
    def __init__(self, question):
        ...
        valor = PreguntasRespuestas.objects.select_related("pregunta").filter(
            pregunta__encuesta=1, pregunta=question
        )
...
class PreguntaBotton(forms.Form):
    ...
    def __init__(self, question, *args, **kwargs):
        ...
        valor = PreguntasRespuestas.objects.prefetch_related("pregunta").filter(
            pregunta__encuesta=2, pregunta=question
        )
        self.fields["respuesta"].queryset = valor
        self.fields["respuesta"].empty_label = None
